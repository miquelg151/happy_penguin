...
for (i in seq_along(matrix_list)) {
	...
	assign(paste0(days[i], "_raw"), raw_object)  # els Seurat objects estan guardats com a J0_raw, etc.                               
}


# QC ---------------------------------------------------------------------------
J0_raw$percent.mt <- PercentageFeatureSet(J0_raw, pattern = "^MT-")
J0_raw$percent.rb <- PercentageFeatureSet(J0_raw, pattern = "^RP[SL]")
J0_counts <- J0_raw@assays$RNA$counts
tsb<- which(grepl("-TsB", rownames(J0_counts)))
cmo3 <- which(grepl("CMO3", rownames(J0_counts)))
...
for (i in seq_along(sub)) {
	...
	sub[[i]] <- FindVariableFeatures(sub[[i]], nfeatures = 5000) ## Note that I put 5000
...
plot3 <- FeaturePlot(J21.cl, features = "percent.mt")
plot4 <- FeaturePlot(J21.cl, features = "percent.rb")
